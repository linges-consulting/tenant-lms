...
| Training Lifecycle | Unpublish was creator-only; send-to-draft allowed manager bypass | Refactored to baton model: Creator owns Draft↔Ready; Manager owns Ready↔Published and Unpublish (resets progress). `/unpublish` now manager-only, returns to Ready. `/send-to-draft` owner-only, blocked when published. BR-301 updated. | — |
| Training Editor | Published training showed no lifecycle button for dual-role users | Disabled "Revert to Draft" button shown with tooltip explaining unpublish-first flow when `is_published=true` | — |
| Training Editor | PDF chapter description edit overwrote `pdf_url` in `content_data` | Fixed `handleSaveChapter` to spread existing `content_data` before setting description, preserving `pdf_url` | — |
| Quiz Engine | Quiz score returned as float (e.g. 66.7%) | `round()` added to score calculation; `score` schema field changed from `float` to `int` | — |
| Audit History | Collaborator add/remove events not shown in timeline | Frontend icon map extended with `COLLABORATOR_ADDED`/`COLLABORATOR_REMOVED`; backend audit query expanded to include chapter/module events via `metadata_json->>'training_id'` | — |
| Learner Dashboard | Stats (Assigned, In Progress, Completed, Overdue) always showed 0 | Backend `/dashboards/employee` rewrote to return the 4 count fields the frontend expects instead of raw training lists | — |
| Manager Dashboard | Overdue and Active Assignments stats showed 0 | Backend `/dashboards/manager` field names aligned with frontend interface (`overdue_count`, `active_assignments`, `total_trainings`) | — |
| Learning Navigation | Learner sidebar had no dedicated "My Trainings" link | Added `My Trainings → /dashboard/my-courses` nav link; wired existing `MyTrainings` component to the route | — |
| Unpublish | No warning when unpublishing a training with completions | `GET /completion-count` endpoint added; frontend shows AlertDialog with employee count before proceeding | — |
| Category Management | No UI to manage training categories | New `ManagerCategoryManager` page at `/manage/categories`; full CRUD (create, rename, delete with confirmation); nav link added | — |
| Quiz Lockouts | No way to identify or reset employees at max attempts | `GET /quiz-attempts-summary` endpoint lists lockouts per chapter; `ManagerReports` page extended with lockout table and per-user Reset button | — |
| Clone Training | No way to duplicate an existing training | `POST /{id}/clone` endpoint deep-copies training + modules + chapters (content_data includes quizzes); Clone option in training list dropdown; opens editor on new draft | — |
| Clone Training | Clone did not preserve modules/chapters — FK ordering issue | Added `await db.flush()` between module and chapter inserts; added `completion_mode` field copy; diagnostic logging added | — |
| Quiz Grading | Correct answers occasionally marked incorrect | Debug logging added to `submit_quiz` — logs `QUIZ_GRADE_MISMATCH` with question ID, type, user answer, expected answer to identify ID mismatch | — |
| Audit Log | Managers saw only state-transition events; creators saw everything — inverted from intended | Inverted role filter: managers see ALL events (including progress_reset, quiz_reset, SUBMIT_QUIZ); creators/collaborators see content + collaborator + state events only | — |
| Unpublish Warning | Dialog only warned about completed users, not currently assigned users | `/completion-count` now returns `{completed_count, assigned_count}`; dialog shows both counts with contextual message | — |
| Audit Log UI | Metadata shown as flat always-visible block | Redesigned as collapsible rows — collapsed = one line; expanded = structured metadata; content-change entries show "View vN snapshot →" link | — |
| Training Editor | History tab: audit log was secondary (1 col), snapshots were primary (2 col) | Swapped: audit timeline is now 2-col primary; snapshots sidebar is compact 1-col list (vN · time · View) | — |
| Training Editor | "View Snapshot" navigated to TrainingViewer in learner mode | Changed to content preview mode: `/manage/learn/{id}?preview=true&mode=content&version={N}` | — |
| Training Editor | No way to set expiry date on a training | Expiry date picker added to settings card (`expires_at`); shown as date input, maps to ISO string | — |
| Training Editor | PDF chapter edit showed blank upload form, losing existing PDF | Edit mode now shows iframe preview of existing PDF + Replace (swap file) + Remove (clear URL) buttons | — |
| My Trainings | No expiry date indication on training cards | Added expiry badge: red "Expired" / amber "Expires in Xd" / muted "Expires {date}" based on `expires_at` | — |
| Training Expiry | No auto-archive for expired trainings | New `POST /trainings/internal/auto-archive-expired` endpoint (X-Internal-Api-Key protected); daily cron in notification_service triggers it at midnight UTC; Alembic migration `b2c3d4e5f6a7` adds `expires_at` column | — |
| Vite Dev Server | Docker gateway hostname blocked by Vite security check | Added `allowedHosts: ['gateway', 'localhost']` to `vite.config.ts` | — |
| PDF Upload | PDF chapter showing 404 in preview | Added debug logging to `_handle_pdf_upload` to confirm `content_data["url"]` is set after save | — |
